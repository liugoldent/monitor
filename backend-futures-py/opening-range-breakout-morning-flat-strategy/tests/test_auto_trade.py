...
class AutoTradeTests(unittest.TestCase):
    def test_login_prefers_strategy_specific_credentials(self) -> None:
        api = Mock()
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(
                os.environ,
                {
                    "OPENING_RETEST_API_KEY": "opening-key",
                    "OPENING_RETEST_SECRET_KEY": "opening-secret",
                    "API_KEY2": "fallback-key",
                    "SECRET_KEY2": "fallback-secret",
        ...
        api.login.assert_called_once_with("opening-key", "opening-secret")

    def test_login_can_fall_back_to_second_account_credentials(self) -> None:
        api = Mock()
        sj = SimpleNamespace(Shioaji=Mock(return_value=api))
        with tempfile.TemporaryDirectory() as directory:
            ca_path = Path(directory) / "Sinopac.pfx"
        ...
        api.login.assert_called_once_with("fallback-key", "fallback-secret")

    def test_rejects_more_than_one_contract_by_default(self) -> None:
        with self.assertRaisesRegex(ValueError, "-1、0或1"):
            auto_trade.execute_target_position(2, api=Mock(), sj=Mock())

    def test_u_two_allows_two_contract_target(self) -> None:
        api = Mock()
        sj = Mock()
        expected = Mock()
        with patch.dict(
