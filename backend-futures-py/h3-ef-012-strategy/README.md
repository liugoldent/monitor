...
- E、F沒有共識、互相衝突或任一組淨部位為0：只跟H `U`口。
- E、F方向一致且與H同向：跟H `2U`口。

E與F的方向分別由各自六套策略的淨部位正負號決定。程式不會把十二套原始口數直接送到永豐。`U`預設為1，因此目前仍是原本的0/1/2口規則。
...
- 每筆有效H或E/F訊號都會各自送出Discord確認；即使同一秒內收到多筆，也不會合併或去重。最終目標未改變時，會附上完整判斷及「維持原部位、不送Discord模擬單」，用來確認監控仍正常收訊。
...
- 實單模組`auto_trade.py`會查詢永豐TMF實際淨部位，依目標差額送一筆IOC市價單；判斷邏輯不放在下單模組。
