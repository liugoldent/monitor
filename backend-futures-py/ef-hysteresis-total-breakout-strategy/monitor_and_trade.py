...
def cycle_date(stamp: datetime):
    return stamp.date() if stamp.time() >= day_time(8, 45) else stamp.date() - timedelta(days=1)
...
def initialize_state(rows) -> dict:
    ...
    state = {
        "schema_version": 1,
        "strategy": "ef_hysteresis_total_breakout_shadow_v1",
...
def clock_flatten(state, now, notify) -> None:
    ...
    state.update({"target": 0, "baseline_cycle": None, "bull_baseline": 0,
                  "bear_baseline": 0, "last_flat_date": date_text})
    persist(state)
    notify("🌅【TOTAL突破策略｜01:00影子清倉】\n"
           f"影子目標：{position_text(previous)} → 空手\n"
           "01:00～08:45繼續更新E/F；開盤後首筆訊號前建立TOTAL基準。\n"
           "本策略不連線券商、不送委託。")
...
def process_rows(state, rows, notify) -> None:
    ...
    for row_number, row in enumerate(rows[count:], start=count + 1):
        ...
        initialized = False
        if not morning and baseline_cycle != cycle:
        ...
        message = (
            ...
            f"開盤前基準：多方{baseline.bull}／空方{baseline.bear}"
...
def main() -> None:
    ...
    try:
        ...
        notifier("✅【開始監控｜EF Hysteresis TOTAL突破】\n"
                 ...
                 "08:45後首筆訊號前固定TOTAL基準；E/F達2/2且方向TOTAL嚴格突破基準才進場。\n"
                 "模式：影子監控，絕不送出券商委託。")
        while True:
