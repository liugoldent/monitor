...
def decide(
    ...
    *,
    ...
    """Apply fixed 2/1 hysteresis and the long re-cross lock."""
    ...
    if bear:
        return Decision(-1, e_net, f_net, long_locked, "空方達到-2/-2，依現行規則進場或反轉")
    if bull:
        if long_locked and current <= 0:
            return Decision(0, e_net, f_net, True, "早盤前多方共識已形成；等待先跌破+2/+2再重新突破")
        return Decision(1, e_net, f_net, long_locked, "多方由未達門檻重新達到+2/+2")
    if current > 0 and e_net >= hold_threshold and f_net >= hold_threshold:
        return Decision(1, e_net, f_net, long_locked, "多單續抱：E/F皆至少+1")
    if current < 0 and e_net <= -hold_threshold and f_net <= -hold_threshold:
        return Decision(-1, e_net, f_net, long_locked, "空單續抱：E/F皆至多-1")
    return Decision(0, e_net, f_net, long_locked, "共識退出")
