...
## 2026-09-21：程序級 Shioaji 長連線（目前行為）

- 使用主帳號 `API_KEY` / `SECRET_KEY`，委託格式維持 TMFR1、市價 MKT、IOC、Auto，`place_order(..., timeout=0)`。
- 監控程序啟動時先完成登入與 CA 啟用，成功後才讀取或消耗訊號；登入失敗時安全待命，直到 `.env` 更新後再嘗試。
- 一般 EF 訊號與 01:00 時鐘清倉共用同一個 Shioaji 實例；每筆委託不重複登入或登出，程序結束時由作業系統回收原生資源。
- 新訊號只查當下券商庫存，以「最終口數 − 券商庫存」計算本次差額；送單後不輪詢成交、不重送。
- API 返回後，先保存委託 ID 與送單紀錄，再保存監控狀態 `submitted`；不因單筆委託銷毀 Shioaji／pysolace 原生連線。
...
- 01:00實單清倉由本機時鐘排程立即對帳，不等待K棒檔案寫入。
- 時鐘排程會記錄預定時間、實際觸發時間、完成時間，以及是否在01:00:30前完成；
  Discord 通知策略開始、新訊號實單執行結果與時鐘清倉。
- 每次E/F判斷通知會列出兩組各六套策略的目前部位與加總，方便核對淨部位來源。
- 正式監控不讀取 `webhook_data_1min.csv`，也不維護影子成交、影子損益或歷史價格重建狀態。
- `backtest.py` 保留為離線策略研究工具；它不在正式監控程序中執行。
...
`records/` 保存實單委託與時鐘清倉稽核；`runtime/` 保存來源 CSV 的實單讀取進度、
原始 E/F 部位、實單目標與重啟狀態。既有影子紀錄檔只作為歷史資料保留，正式監控不再追加。
...
正式入口固定為實單模式。啟動先建立程序級 Shioaji 長連線，再讀取訊號；啟動不補單，等待啟動後的新 EF 訊號。一般訊號與 01:00 時鐘清倉共用該連線。每筆新訊號依券商實際庫存調整到策略目標，已符合目標則不送單。同一筆訊號不重送；下一筆新訊號即使目標相同，也照常查庫存並處理。失敗或中斷只留下紀錄與通知，不鎖住新訊號或清倉。
