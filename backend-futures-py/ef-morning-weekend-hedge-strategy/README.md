...
若重啟發現 `attempt.status` 還是 `attempted`，代表上次在送單途中中斷，會設定 `blocked_reason`。若中斷的是01:00清倉，新時段重設會將它轉為人工清倉待辦並解除該次暫停；一般訊號委託中斷的保護仍保留。人工處理的券商殘留部位與新時段策略 JSON 分開，JSON 歸零不代表券商空手。
