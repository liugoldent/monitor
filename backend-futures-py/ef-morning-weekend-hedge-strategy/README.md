...
TMFR1 / MKT / IOC / Auto 格式不變。API 返回後，在 `logout()` 前原子保存
`submitted` 或 `no_order_needed`，避免登出中斷讓已返回的委託停留在 `attempted`。
`submitted` 只代表送單呼叫返回，不保證券商受理或成交；不回查、不重送。
送單、API 返回及登出均有分段 log，主程式啟用 faulthandler。
若在 API 返回及持久化之前中斷，結果仍然不明，保留原本的中斷保護。
...
- 新訊號送單後的 JSON 合計淨部位限制為 -2～+2 口 TMF（含邊界，計入口數倍率）。超過上限只發 Discord、不送單，保留該策略原部位並消耗訊號，不補單；未超過且差額非零則照常通知及送單。01:00 清倉不受上限限制，仍查實際庫存、通知及送清倉單。上限依 JSON 追蹤部位計算，不是券商實際庫存保證。
...
一般訊號委託中斷後，交易時段內只做一次唯讀核對：登入永豐2、等1秒、更新委託狀態、查詢 TMF 庫存。TMFR1 對應月份的實際有號口數符合 JSON 目標且沒有未結 TMF 委託時，保存為 `broker_reconciled`；不一致、有未結委託或查詢失敗則保存為 `recovery_skipped_no_retry`，只通知一次。兩者均解除該次中斷暫停，保留 JSON 部位，跳過中斷那筆、不重送，後續新訊號繼續處理。既有已跳過訊號與啟動前訊號仍不補單。JSON 淨部位±2口限制保留；下單商品固定為微型台指期 TMFR1。

送單前將 positions、新訊號進度、attempt 一起原子保存。一般委託只送一次，不查成交、不重試。JSON 是策略追蹤部位，不是成交證明；拒單、未成交、手動交易仍可能造成券商實際庫存差異。

若重啟發現 `attempt.status` 還是 `attempted`，代表上次在送單途中中斷，會設定 `blocked_reason`。若中斷的是01:00清倉，新時段重設會將它轉為人工清倉待辦並解除該次暫停；一般訊號委託中斷則依上述單次核對流程解除暫停。人工處理的券商殘留部位與新時段策略 JSON 分開，JSON 歸零不代表券商空手。
...
正式入口固定使用 API_KEY2 永豐實單。Discord 通知策略開始、訊號委託及 01:00 清倉。每筆訊號通知包含來源策略名稱、代碼及多／空進場、出場或反向動作；未提供名稱時顯示代碼。超限、零差額及暫停下單的通知同樣列出来源訊號。訊號原始部位與 JSON 追蹤部位分開，避免重啟後誤判訊號動作。委託已送出不表示已成交。
